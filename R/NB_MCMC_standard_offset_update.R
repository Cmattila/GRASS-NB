...
VS_Standard_offset <- function(K, y, NeighborhoodList, which.prior,niter, verbose, pop_col, scale){
  ...
  if(which.prior == "HS" & !is.null(NeighborhoodList)){
    ...
    for (i in 1:niter){
      ...
      ## Algorithm 1st step: update the predictor function and probability vector q ----
      #============================================================================#
      as.vector(K_sp %*% beta) + rep(phi,1) + log(pop_col) - log(scale)  #modeling rates with offset for population and scale
    ...
    for (i in 1:niter){
      ...
      ## Algorithm 1st step: update the predictor function and probability vector q ----
      #============================================================================#
      eta <- as.vector(K_sp%*%beta) + log(pop_col) - log(scale)  #modeling rates with offset for population and scale
