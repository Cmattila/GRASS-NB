...
VS_Standard <- function(K, y, NeighborhoodList, which.prior,niter, verbose, pop_col, scale){
 ...
 # Starting VS algorithms
  if(verbose == "TRUE" & which.prior == "HS" & !is.null(NeighborhoodList)){print(paste0("MUSC: NB model fitting has started with standard HS prior for spatial count data."))}
  if(verbose == "TRUE" & which.prior == "SS" & !is.null(NeighborhoodList)){print(paste0("MUSC: NB model fitting has started with standard SS prior for spatial count data."))}
  if(verbose == "TRUE" & which.prior == "HS" & is.null(NeighborhoodList)){print(paste0("MUSC: NB model fitting has started with standard HS prior for aspatial count data"))}
  if(verbose == "TRUE" & which.prior == "SS" & is.null(NeighborhoodList)){print(paste0("MUSC: NB model fitting has started with standard SS prior for aspatial count data"))}
  ...
  if(which.prior == "HS" & !is.null(NeighborhoodList)){
    ...
    for (i in 1:niter){
      ...
      #============================================================================#
      # eta <- K%*%beta + rep(phi,1) # modeling counts
      ...
      # Prior precision (diagonal) with nugget
      prior_diag  <- c(T0, 1 / (Lambda2 * tau2)) + nugget
      Q_prior_sp  <- spam::diag.spam(prior_diag)          # p x p sparse

      # Precompute sqrt(w)
      sqrtw <- sqrt(w)

      # Build scaled K: K_scaled = diag(sqrt(w)) %*% K
      K_scaled_sp <- spam::diag.spam(sqrtw) %*% K_sp      # N x p, sparse

      # Likelihood precision: (K_scaled)' K_scaled
      Q_lik_sp <- crossprod(K_scaled_sp)                  # p x p sparse

      # Total precision
      Q_beta_sp <- Q_prior_sp + Q_lik_sp

      # Canonical mean term:
      # b = (K sqrt(w))' (sqrt(w) * (z - phi))
      b_beta <- as.vector(crossprod(K_scaled_sp, sqrtw * (z - phi)))

      # Draw beta ~ N(Q^{-1} b, Q^{-1})
      ...
      # Data contribution: diag(w) as spam
      W_phi_sp <- spam::diag.spam(w)
      prec_sp  <- priorprec_sp + W_phi_sp               # nspace x nspace (spam)

      # Canonical mean term: w * (z - K beta)
      ...
      # Tauphi update: use spam Q for speed
      quad_phi <- as.numeric(crossprod(phi, Q_sp %*% phi))
      tauphi <- rgamma(1, 0.1 + (nspace - 1) / 2, 0.1 + quad_phi / 2)  # n-1 since rank of Q is n-1
      sphi <- sqrt(1 / tauphi)
    ...
    for (i in 1:niter){
      ...
      ## Algorithm 1st step: update the predictor function and probability vector q ----
      #============================================================================#
      eta <- as.vector(K_sp %*% beta) + phi   # modeling counts
    ...
    for (i in 1:niter){
      ...
      # sqrt(w) trick for speed: K_scaled = diag(sqrt(w)) %*% K
      sqrtw       <- sqrt(w)
      K_scaled_sp <- spam::diag.spam(sqrtw) %*% K_sp   # N x p sparse

      # Likelihood precision: (K_scaled)' K_scaled = K' diag(w) K
      Q_lik_sp    <- crossprod(K_scaled_sp)            # p x p sparse

      # Total precision for beta
      Q_beta_sp   <- Q_prior_sp + Q_lik_sp

      # Canonical mean term: (K sqrt(w))' (sqrt(w) * z) = K' diag(w) z
      b_beta      <- as.vector(crossprod(K_scaled_sp, sqrtw * z))

      # Draw beta ~ N(Q^{-1} b, Q^{-1}) in canonical form (no as.matrix)
      beta <- c(spam::rmvnorm.canonical(1, b_beta, Q_beta_sp))
  ...
  if(verbose == "TRUE" & which.prior == "HS" & !is.null(NeighborhoodList)){print(paste0("MUSC: NB model fitting with standard HS prior for spatial count data completed!"))}
  if(verbose == "TRUE" & which.prior == "SS" & !is.null(NeighborhoodList)){print(paste0("MUSC: NB model fitting with standard SS prior for spatial count data completed!"))}
  if(verbose == "TRUE" & which.prior == "HS" & is.null(NeighborhoodList)){print(paste0("MUSC: NB model fitting with standard HS prior for aspatial count data completed!"))}
  if(verbose == "TRUE" & which.prior == "SS" & is.null(NeighborhoodList)){print(paste0("MUSC: NB model fitting with standard SS prior for aspatial count data completed!"))}
